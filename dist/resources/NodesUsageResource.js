 ...
 */
function marshalNodeUsage(snapshot) {
    const obj = {};
    // 节点基本信息
...
class NodesUsageResource extends MCPResource {
    ...
    name = "节点资源用量";
    description = "返回当前所有节点的使用情况和状态信息，包含云主机状态、核心利用率、内存利用率、磁盘利用率、网络情况和系统状态。";
    ...
    async read() {
        ...
        try {
            ...
            const nodesUsage = nodesUsageResult.data;
            // 构建返回多个resource对象
            const resourcesList = nodesUsage.map((nodeUsage) => {
            ...
            const output = `获取节点使用情况失败：${error instanceof Error ? error.message : String(error)}`;
            logger.error(output);
            return [
                {
                    uri: "",
