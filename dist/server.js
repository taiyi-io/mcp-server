...
import { z } from "zod";
import { newInsecureConnector, ResourceType, } from "@taiyi-io/api-connector-ts";
import os from "os";
...
const connector = await newInsecureConnector(deviceID, accessString, backendHost, backendPort);
const server = new McpServer({
    name: "taiyi-cloud",
    version: "0.11.0",
});
// 注册磁盘镜像列表资源
server.registerResource("disk-images", "disk-images://", {
    title: "磁盘镜像列表",
    ...
    try {
        // 第一次请求获取总数
        const firstResponse = await connector.queryDiskFiles(offset, pageSize, false);
        ...
        const resourcesList = allImages.map((image) => {
            // 格式化text内容
            let text = `id:${image.id} 名称:${image.name} 描述:${image.description || "无"}`;
            ...
            return {
                uri: `taiyi://disk-images/${image.id}`,
                text: text,
            };
        });
        return {
...
// 注册单个磁盘镜像资源
server.registerResource("disk-image-detail", new ResourceTemplate("disk-images://{id}/detail", { list: undefined }), {
    title: "磁盘镜像详情",
            ...
            contents: [
                {
                    uri: uri.href,
                    mimeType: "text/plain",
                    text: `获取磁盘镜像 ${id} 详情失败：${error instanceof Error ? error.message : String(error)}`,
                },
            ],
        };
    }
...
// 注册标记系统磁盘镜像工具
server.registerTool("markSystemDiskImage", {
    title: "标记系统磁盘镜像",
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `标记系统磁盘镜像失败：${error instanceof Error ? error.message : String(error)}`,
                },
            ],
        };
    }
...
// 注册取消标记系统磁盘镜像工具
server.registerTool("unmarkSystemDiskImage", {
    title: "取消系统磁盘镜像标记",
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `取消系统磁盘镜像标记失败：${error instanceof Error ? error.message : String(error)}`,
                },
            ],
        };
    }
...
// 注册标记系统磁盘镜像提示词
server.registerPrompt("markSystemDiskImagePrompt", {
    title: "标记系统磁盘镜像",
...
// 注册取消标记系统磁盘镜像提示词
server.registerPrompt("unmarkSystemDiskImagePrompt", {
    title: "取消系统磁盘镜像标记",
