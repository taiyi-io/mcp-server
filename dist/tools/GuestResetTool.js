...
class GuestResetTool extends MCPTool {
    name = "guest-reset";
    description = "强制重启云主机，强制重启可能会导致云主机数据丢失，请谨慎操作。如果仅知道云主机名称，可以通过mcp-tool:find-guest-id-by-name获取云主机id，再调用本方法";
    schema = {
        guestID: {
            type: z.string(),
    ...
    async execute(input) {
        try {
            ...
            const output = `强制重启云主机失败：${error instanceof Error ? error.message : String(error)}`;
            logger.error(output);
            return output;
        }
    }
