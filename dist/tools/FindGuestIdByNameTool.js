...
class FindGuestIdByNameTool extends MCPTool {
    name = "find-guest-id-by-name";
    description = "通过云主机名称查找云主机ID。如果需要调用云主机接口，只有云主机名称而没有云主机id时，通过本接口获得正确id再调用目标tool";
    schema = {
        name: {
            type: z.string(),
            description: "云主机名称",
    ...
    async execute(input) {
        try {
            ...
            const output = error instanceof Error ? error.message : "查找云主机ID失败";
            logger.error(output);
            return output;
        }
    }
