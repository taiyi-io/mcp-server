...
class ListGuestsTool extends MCPTool {
    ...
    schema = {
        ...
        state: {
            type: z.string().optional(),
            description: "云主机状态：stopped、starting、running、stopping、suspending、suspended",
        },
    };
    async execute(input) {
        try {
            ...
            const guests = await queryGuests(connector, input.selfOnly || false, input.keywords || [], input.pool, input.node, targetState);
            const formattedGuests = guests.map((guest) => marshalGuestView(guest));
            return `系统当前共有 ${guests.length} 个符合条件的云主机:\n ${formattedGuests.join("\n")}`;
        }
        catch (error) {
