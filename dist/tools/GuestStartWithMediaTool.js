...
class GuestStartWithMediaTool extends MCPTool {
    ...
    schema = {
        ...
        mediaID: {
            type: z.string(),
            description: "ISO镜像ID，如果仅有名称，可以通过mcp-tool:find-iso-image-id-by-name获取ID",
        },
        sync: {
    ...
    async execute(input) {
        try {
            ...
            const result = await connector.tryStartGuest(input.guestID, input.mediaID);
            if (result.error) {
