...
class MarkSystemDiskImageTool extends MCPTool {
    ...
    description = "根据指定id将磁盘镜像标记为系统磁盘镜像。可以通过mcp-tool:find-disk-image-id-by-name输入磁盘镜像名称获得磁盘镜像ID，再调用本接口";
    schema = {
        imageID: {
            type: z.string(),
    ...
    async execute(input) {
        try {
            ...
            await connector.setSystemResource(ResourceType.DiskImage, input.imageID, true);
            return `成功将磁盘镜像 ${input.imageID} 标记为系统磁盘镜像`;
        }
        catch (error) {
            const output = `标记系统磁盘镜像失败：${error instanceof Error ? error.message : String(error)}`;
            logger.error(output);
            return output;
        }
    }
