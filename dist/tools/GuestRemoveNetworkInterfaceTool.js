...
class GuestRemoveNetworkInterfaceTool extends MCPTool {
    ...
    async execute(input) {
        try {
            ...
            if (isExternal) {
                const result = await connector.removeExternalInterface(input.guestID, input.macAddress);
                if (result.error) {
                ...
                const result = await connector.removeInternalInterface(input.guestID, input.macAddress);
                if (result.error) {
                    throw new Error(result.error);
                }
            ...
            const output = `删除网卡失败：${error instanceof Error ? error.message : String(error)}`;
            logger.error(output);
            return output;
        }
    }
