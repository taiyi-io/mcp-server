...
class CreateDiskImageTool extends MCPTool {
    ...
    schema = {
        ...
        access_level: {
            ...
            description: "资源访问级别:private:私有<默认>, share_view:共享查看, share_edit:共享编辑, global_view:全局可见",
    ...
    async execute(input) {
        try {
            ...
            // 调用connector创建磁盘镜像
            const result = await connector.tryCreateDiskImage(input.guestID, "sys", spec, input.access_level);
            if (result.error) {
