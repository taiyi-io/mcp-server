...
class GuestAddNetworkInterfaceTool extends MCPTool {
    name = "guest-add-network-interface";
    description = "指定云主机ID，添加网络接口到云主机。如果仅知道云主机名称，可以通过mcp-tool:find-guest-id-by-name获取云主机id，再调用本方法";
    schema = {
        guestID: {
            type: z.string(),
        ...
        macAddress: {
            type: z
                ...
                .optional(),
            description: "目标网卡的MAC地址，格式为 xx:xx:xx:xx:xx:xx，未提供时将自动生成",
        },
        external: {
    ...
    async execute(input) {
        try {
            ...
            if (isExternal) {
                const result = await connector.addExternalInterface(input.guestID, macAddress);
                if (result.error) {
                ...
                const result = await connector.addInternalInterface(input.guestID, macAddress);
                if (result.error) {
                    throw new Error(result.error);
                }
            ...
            const output = `添加网卡失败：${error instanceof Error ? error.message : String(error)}`;
            logger.error(output);
            return output;
        }
    }
