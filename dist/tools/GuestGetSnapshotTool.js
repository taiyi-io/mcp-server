...
function marshalSnapshotData(snapshot) {
    const snapshotData = {};
    // 映射字段
...
class GuestGetSnapshotTool extends MCPTool {
    name = "guest-get-snapshot";
    description = "根据指定ID获取云主机快照详情，包含标识、名称、创建时间、实时快照、当前快照、描述等信息";
    schema = {
        guestID: {
            type: z.string(),
    ...
    async execute(input) {
        try {
            ...
            const result = await connector.getSnapshot(input.guestID, input.snapshotID);
            if (result.error) {
            ...
            const output = `获取云主机 ${input.guestID} 快照 ${input.snapshotID} 详情失败：${error instanceof Error ? error.message : String(error)}`;
            logger.error(output);
            return output;
        }
    }
