...
class GuestRemoveVolumeTool extends MCPTool {
    name = "guest-remove-volume";
    description = "指定云主机ID和磁盘标签，删除数据磁盘。不支持删除系统磁盘。如果仅知道云主机名称，可以通过mcp-tool:find-guest-id-by-name获取云主机id，再调用本方法";
    schema = {
        guestID: {
            type: z.string(),
        ...
        tag: {
            ...
            description: "要删除的目标磁盘标签，从云主机详情或者云主机列表中获取，只能删除data_开头的数据磁盘",
        },
    };
    async execute(input) {
        try {
            ...
            const output = `删除数据磁盘失败：${error instanceof Error ? error.message : String(error)}`;
            logger.error(output);
            return output;
        }
    }
