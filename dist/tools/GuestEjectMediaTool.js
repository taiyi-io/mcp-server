...
class GuestEjectMediaTool extends MCPTool {
    name = "guest-eject-media";
    description = "卸载云主机的ISO媒体。如果仅知道云主机名称，可以通过mcp-tool:find-guest-id-by-name获取云主机id，再调用本方法";
    schema = {
        guestID: {
            type: z.string(),
    ...
    async execute(input) {
        try {
            ...
            const output = `卸载媒体失败：${error instanceof Error ? error.message : String(error)}`;
            logger.error(output);
            return output;
        }
    }
