...
class SearchGuestsTool extends MCPTool {
    name = "search-guests";
    description = "根据关键词搜索云主机，返回名称、存储池、地址池、主机ID、主机地址、所在池以及网络接口信息匹配任一关键词的云主机";
    schema = {
        keywords: {
            type: z.array(z.string()).nonempty(),
            description: "关键词集合，不可为空，用于匹配云主机的名称、存储池、地址池、主机ID、主机地址、所在池以及网络接口信息",
        },
        selfOnly: {
    ...
    async execute(input) {
        try {
            ...
            const filteredGuests = guests.filter((guest) => {
                // 构建搜索文本，包含基本属性
                let searchText = `${guest.name || ""} ${guest.storage_pool || ""} ${guest.address_pool || ""} ${guest.host_id || ""} ${guest.host_address || ""} ${guest.pool || ""}`.toLowerCase();
                // 添加网络接口信息到搜索文本
                if (guest.network_interfaces) {
                    // 处理内部网络接口
                    if (guest.network_interfaces.internal) {
                        guest.network_interfaces.internal.forEach((iface) => {
                            searchText += ` ${iface.mac_address || ""} ${iface.ip_address || ""} ${iface.ip_address_v6 || ""}`;
                        });
                    }
                    // 处理外部网络接口
            ...
            // 格式化输出结果
            const formattedGuests = filteredGuests.map((guest) => marshalGuestView(guest));
            return `找到 ${filteredGuests.length} 个匹配关键词 "${input.keywords.join(", ")}" 的云主机：\n${formattedGuests.join("\n")}`;
        }
        catch (error) {
            return `搜索云主机失败：${error instanceof Error ? error.message : String(error)}`;
        }
    }
}
