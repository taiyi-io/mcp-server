...
class GuestRestoreSnapshotTool extends MCPTool {
    ...
    async execute(input) {
        try {
            ...
            // 调用connector恢复云主机快照
            const result = await connector.tryRestoreSnapshot(input.guestID, input.snapshotID);
            if (result.error) {
            ...
            const output = `恢复云主机快照失败: ${error instanceof Error ? error.message : String(error)}`;
            logger.error(output);
            return output;
        }
    }
