...
function marshalUsageDate(
  usage: GuestResourceUsageData,
  spec: GuestSpec
): string {
  const obj: { [key: string]: any } = {};
  const totalDisk = spec.disks.reduce((acc, disk) => acc + (disk.size || 0), 0);
  //usage的核心、内存、磁盘均为百分比利用率，0~100
  obj["CPU负载"] = `${((usage.cores * spec.cores) / 100).toFixed(2)} / ${
    spec.cores
  } 核心`;
  obj["内存负载"] = `${((usage.memory * spec.memory) / 100).toFixed(2)} / ${
    spec.memory
  } MB`;
  obj["磁盘负载"] = `${((usage.disk * totalDisk) / 100 / 1024).toFixed(2)} / ${
    totalDisk / 1024
  } GB`;
  ...
  obj["网络带宽"] = marshalNetworkSpeed(
    usage.receivedBytesPerSecond,
    usage.transmittedBytesPerSecond
  );
...
class GetGuestRealtimeUsageTool extends MCPTool<GetGuestRealtimeUsageInput> {
  name = "get-guest-realtime-usage";
  description =
    "获取云主机的实时资源使用情况，包含CPU、内存、磁盘负载和磁盘读写速度，网络收发带宽等信息，通常用于监控云主机的实时运行状态，仅限运行中云主机调用。如果仅知道云主机名称，可以通过mcp-tool:find-guest-id-by-name获取云主机id，再调用本方法";
  ...
  async execute(input: GetGuestRealtimeUsageInput) {
    try {
