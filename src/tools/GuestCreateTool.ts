...
class GuestCreateTool extends MCPTool<GuestCreateInput> {
  name = "guest-create";
  description =
    "创建云主机，支持同步和异步操作，可指定资源池、系统模板、核心数、内存、磁盘等配置";

  schema = {
    poolID: {
      type: z.string(),
      description:
        "承载云主机的资源池id，必填。可以从mcp-resource://compute-pool/中获取",
    },
    system: {
      type: z.string().optional(),
      description:
        "云主机创建时使用的系统模板id，能够针对性优化云主机配置参数提高更好的性能与兼容性，可选。不指定时使用默认配置。可以从mcp-resource://system/中获取当前可用系统模板",
    },
    name: {
      type: z.string(),
    ...
    cores: {
      type: z.string(),
      description:
        "云主机核心数量，1或者2的倍数。不能够超过目标资源池最大核心数，目标资源池允许的最大核心数可以通过mcp-resource://compute-pool/获取",
    },
    memory: {
      type: z.string(),
      description:
        "以MB为单位的内存容量，比如2GB内存，输入为2048，必填。不能够超过目标资源池最大内存，目标资源池允许的最大内存可以通过mcp-resource://compute-pool/获取",
    },
    disks: {
      type: z.string(),
      description:
        '以MB为单位的磁盘容量，[系统盘,数据盘1，数据盘2]的形式输入，必填。比如创建1G系统磁盘，输入"[1024]";创建2G系统盘，5G数据盘，输入"[2048,5120]";申请磁盘容量不能超过目标资源池限制，目标资源池允许的最大磁盘容量可以通过mcp-resource://compute-pool/获取',
    },
    source_image: {
      type: z.string().optional(),
      description:
        "克隆创建云主机时的来源磁盘镜像id，创建时自动将磁盘镜像数据复制到新建云主机的系统磁盘，系统磁盘容量必须不小于镜像要求的磁盘空间，为空时创建空白云主机。可用磁盘镜像可以通过mcp-resource://disk-image/获取",
    },
    auto_start: {
      type: z.boolean().optional(),
      description:
        "云主机开机启动，启用后，服务器启动时自动将云主机开机。默认关闭。",
      default: false,
    },
    cloud_init: {
      type: z.boolean().optional(),
      description:
        "是否基于CloudInit对云主机进行初始化配置，包括安全的管理员密码和磁盘挂载等功能，需要磁盘镜像的配合，默认关闭。",
      default: false,
    },
    access_level: {
      type: z.nativeEnum(ResourceAccessLevel),
      description:
        "资源访问级别:private:私有<默认>, share_view:共享查看, share_edit:共享编辑",
      default: ResourceAccessLevel.Private,
    },
  };

  async execute(input: GuestCreateInput) {
    try {
      ...
      if (!/^[a-zA-Z][a-zA-Z0-9-]*$/.test(input.name)) {
        throw new Error(
          "云主机名称格式错误：必须以字母开头，只能包含字母、数字和连字符"
        );
      }
      ...
      if (coresNum > targetPool.resource.cores) {
        throw new Error(
          `核心数量超过目标资源池最大核心数：${targetPool.resource.cores}`
        );
      }
      //check memory
      if (memoryNum > targetPool.resource.memory) {
        throw new Error(
          `内存容量超过目标资源池最大内存：${targetPool.resource.memory}`
        );
      }
      //check disk
      for (const diskSize of disksArray) {
        if (diskSize > targetPool.resource.disk) {
          throw new Error(
            `磁盘容量超过目标资源池最大磁盘：${targetPool.resource.disk}`
          );
        }
      }
      ...
      if (input.source_image) {
        ...
        if (
          sourceImage.volume_size_in_mb &&
          sourceImage.volume_size_in_mb > systemDiskSize
        ) {
          throw new Error(
            `系统磁盘容量不能小于镜像要求的磁盘空间：${sourceImage.volume_size_in_mb}MB`
          );
        }
      }
      ...
      if (input.cloud_init) {
        const cloudInit: ControlCloudInitConfig = {
      ...
      // 6. 调用tryCreateGuest
      const result = await connector.tryCreateGuest(
        input.poolID,
        input.system || "",
        guestConfig
      );
