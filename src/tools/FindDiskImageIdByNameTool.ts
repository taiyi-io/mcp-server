...
class FindDiskImageIdByNameTool extends MCPTool<FindDiskImageIdByNameInput> {
  name = "find-disk-image-id-by-name";
  description =
    "通过磁盘镜像名称查找磁盘镜像ID。如果需要调用磁盘镜像接口，只有镜像名称而没有指定id时，通过本接口获得正确id再调用目标tool";
  ...
  async execute(input: FindDiskImageIdByNameInput) {
    try {
      ...
      const output =
        error instanceof Error ? error.message : "查找磁盘镜像ID失败";
      logger.error(output);
      return output;
    }
  }
