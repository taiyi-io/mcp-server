...
class GuestModifyAutostartTool extends MCPTool<GuestModifyAutostartInput> {
  name = "guest-modify-autostart";
  description =
    "指定云主机ID，启用或禁用云主机的自动启动功能。如果仅知道云主机名称，可以通过mcp-tool:find-guest-id-by-name获取云主机id，再调用本方法";
  ...
  async execute(input: GuestModifyAutostartInput) {
    try {
      ...
      return `成功${enable ? "启用" : "禁用"}云主机 ${
        input.guestID
      } 的自动启动功能`;
    } catch (error) {
      const output = `修改云主机自动启动功能失败：${
