...
class GuestModifyMemoryTool extends MCPTool<GuestModifyMemoryInput> {
  name = "guest-modify-memory";
  description =
    "指定云主机ID和目标内存大小，修改云主机的内存容量，内存大小必须为2的倍数（MB）。如果仅知道云主机名称，可以通过mcp-tool:find-guest-id-by-name获取云主机id，再调用本方法";
  ...
  async execute(input: GuestModifyMemoryInput) {
    try {
      ...
      const result = await connector.modifyGuestMemory(
        input.guestID,
        memoryInMB
      );
