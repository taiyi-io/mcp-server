...
class GuestHardShutdownTool extends MCPTool<GuestHardShutdownInput> {
  name = "guest-hard-shutdown";
  description =
    "强制关闭云主机，强制关闭可能会导致云主机数据丢失，请谨慎操作。如果仅知道云主机名称，可以通过mcp-tool:find-guest-id-by-name获取云主机id，再调用本方法";
  ...
  async execute(input: GuestHardShutdownInput) {
    try {
