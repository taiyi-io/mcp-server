...
class GuestInsertMediaTool extends MCPTool<InsertMediaInput> {
  name = "guest-insert-media";
  description =
    "为云主机加载ISO媒体。如果仅知道云主机名称，可以通过mcp-tool:find-guest-id-by-name获取云主机id；如果仅知道ISO镜像名称，可以通过mcp-tool:find-iso-image-id-by-name获取ISO镜像id，再调用本方法";
  ...
  async execute(input: InsertMediaInput) {
    try {
