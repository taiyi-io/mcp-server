...
function marshalStoragePool(pool: StoragePool): string {
  ...
  if (
    ...
    pool.containers.forEach((container, index) => {
      ...
      if (container.used_size && container.max_size) {
        text += `，已使用 ${(container.used_size / 1024).toFixed(
          2
        )} / ${Math.ceil(container.used_size / 1024)} GB`;
      }
...
class GetStorageDetailTool extends MCPTool<GetStorageDetailInput> {
  ...
  async execute(input: GetStorageDetailInput) {
    try {
      ...
      const pool: StoragePool = result.data;
      const text = marshalStoragePool(pool);
      return text;
    } catch (error) {
      const output = `获取存储池 ${input.storagePoolID} 详情失败：${
