...
export function marshalTaskStatus(task: TaskData): string {
...
export function marshalComputePool(pool: ComputePoolStatus): string {
  const obj: { [key: string]: any } = {};
  obj["标识"] = pool.id;
  if (pool.description) {
...
export function marshalSystemView(view: GuestSystemView): string {
  const obj: { [key: string]: any } = {};
...
export function marshalNodeData(node: ClusterNodeData): string {
...
/**
   ...
   */
 export async function getAllTasks(connector: TaiyiConnector): Promise<TaskData[]> {
  ...
  export function marshalStoragePool(pool: StoragePoolListRecord): string {
  const obj: { [key: string]: any } = {};
  obj["标识"] = pool.id;
  obj["类型"] = pool.type;
