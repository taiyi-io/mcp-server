 ...
 */
function marshalClusterUsage(snapshot: ClusterResourceSnapshot): string {
  ...
  obj[
    "资源池"
  ] = `启用 ${snapshot.pool_enabled} /禁用 ${snapshot.pool_disabled}`;
  ...
  obj[
    "云主机"
  ] = `运行 ${snapshot.guest_running} /停止 ${snapshot.guest_stopped}`;
...
class ClusterUsageResource extends MCPResource {
  ...
  name = "集群资源用量";
  description =
    "返回当前集群的整体使用情况和状态信息，包含节点状态、资源池状态、运行时间、云主机状态、核心利用率、内存利用率、磁盘利用率和系统状态。";
  ...
  async read(): Promise<ResourceContent[]> {
    ...
    try {
      ...
      const snapshot: ClusterResourceSnapshot = result.data;
