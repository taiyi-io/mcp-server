...
import { GuestView, TaiyiConnector } from "@taiyi-io/api-connector-ts";
...
class GuestListResource extends MCPResource {
  ...
  name = "云主机列表";
  description =
    "返回当前用户可以访问的所有云主机列表，包含主机名、标识、核心数、内存、磁盘配置和权限信息。";
  mimeType = "application/json";

  async read(): Promise<ResourceContent[]> {
    const connector: TaiyiConnector = await getConnector();
    try {
      const allGuests: GuestView[] = await queryGuests(connector, false);
      // 处理云主机数据
      const resources: ResourceContent[] = [];
      allGuests.forEach((guest: GuestView) => {
