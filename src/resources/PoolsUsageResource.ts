 ...
 */
function marshalPoolUsage(snapshot: PoolResourceSnapshot): string {
  ...
  obj[
    "节点"
  ] = `在线 ${snapshot.node_online} /离线 ${snapshot.node_offline} /丢失 ${snapshot.node_lost}`;
...
class PoolsUsageResource extends MCPResource {
  ...
  name = "资源池资源用量";
  description =
    "返回当前所有计算资源池的使用情况和状态信息，包含节点状态、云主机状态、核心利用率、内存利用率、磁盘利用率、网络情况和系统状态。";
  ...
  async read(): Promise<ResourceContent[]> {
    ...
    try {
      ...
      const computePools: ComputePoolStatus[] = computePoolsResult.data;
      const poolIds: string[] = computePools.map((pool) => pool.id);
      ...
      const poolsUsage: PoolResourceSnapshot[] = poolsUsageResult.data;

      // 构建返回多个resource对象
      const resourcesList: ResourceContent[] = poolsUsage.map((poolUsage) => {
